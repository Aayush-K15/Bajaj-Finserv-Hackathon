...
def window_kwargs_to_pandas_equivalent(
    ...
    if function_name == "shift":
    ...
    elif function_name.startswith("cum_"):  
        pandas_kwargs = {"skipna": True}
    elif function_name.startswith("rolling_"):  
        ...
        assert "quantile" in kwargs  
        assert "interpolation" in kwargs  
        pandas_kwargs = {
            "q": kwargs["quantile"],
            "interpolation": kwargs["interpolation"],
        }
    elif function_name.startswith("ewm_"):
        assert "com" in kwargs  
        assert "span" in kwargs  
        assert "half_life" in kwargs  
        assert "alpha" in kwargs  
        assert "adjust" in kwargs  
        assert "min_samples" in kwargs  
        assert "ignore_nulls" in kwargs  
        ...
        if not partition_by:
            ...
            def func(df: PandasLikeDataFrame) -> Sequence[PandasLikeSeries]:  
                ...
                if function_name.startswith("cum_"):
                    ...
                    assert "reverse" not in self._scalar_kwargs  
                    reverse = False
                ...
                if function_name.startswith("rolling"):
                    ...
                    assert pandas_function_name is not None  
