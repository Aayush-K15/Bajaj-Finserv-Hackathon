...
class PdfWriter(PdfDocCommon):
    ...
    def _info(self, value: Optional[Union[IndirectObject, DictionaryObject]]) -> None:
        if value is None:
            try:
                self._objects[self._info_obj.indirect_reference.idnum - 1] = None  
    ...
    def _add_object(self, obj: PdfObject) -> IndirectObject:
        if (
            ...
            and obj.indirect_reference.pdf == self  
        ):
    ...
    def _update_field_annotation(
        ...
        if AA.AP not in annotation:
            ...
            n = annotation[AA.AP]["/N"].indirect_reference.idnum  
    ...
    def update_page_form_field_values(
        ...
        for annotation in page[PG.ANNOTS]:  
            ...
            for field, value in fields.items():
                ...
                if not (value is None and flatten):  
    ...
    def clone_reader_document_root(self, reader: PdfReader) -> None:
        ...
        assert len(self._objects) <= cast(int, reader.trailer["/Size"])  
    ...
    def _write_increment(self, stream: StreamType) -> None:
        ...
        if self._info is not None and (
            self._info.indirect_reference.idnum - 1  
            >= len(self._original_hash)
            or cast(IndirectObject, self._info).hash_bin()  
            != self._original_hash[
                self._info.indirect_reference.idnum - 1  
            ]
        ):
        ...
        def clean_forms(
            ...
            if isinstance(elt, StreamObject):  
                if not isinstance(elt, ContentStream):  
                    ...
                    e.update(elt.items())
                    elt = e
                clean(elt, images, forms, text_filters)  
            return images, forms
    ...
    def merge(
        ...
        for page in pages:
            ...
            if position is None:
                ...
                srcpages[pg.indirect_reference.idnum] = self.add_page(
                    pg, [*list(excluded_fields), 1, "/B", 1, "/Annots"]  
                )
            else:
                srcpages[pg.indirect_reference.idnum] = self.insert_page(
                    pg, position, [*list(excluded_fields), 1, "/B", 1, "/Annots"]  
                )
                position += 1
        ...
        if "/AcroForm" in _ro and _ro["/AcroForm"] is not None:
            ...
            try:
                for f in reader.root_object["/AcroForm"]["/Fields"]:  
                    try:
