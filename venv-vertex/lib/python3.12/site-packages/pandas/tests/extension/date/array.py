...
class DateArray(ExtensionArray):
    def __init__(
        ...
        if isinstance(dates, list):
            ...
            self._year = np.zeros(ldates, dtype=np.uint16)  
            self._month = np.zeros(ldates, dtype=np.uint8)  
            self._day = np.zeros(ldates, dtype=np.uint8)  
